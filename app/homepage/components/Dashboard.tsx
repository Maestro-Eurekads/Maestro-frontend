...
import DashBoardChannelDistributionChat from "components/ChannelDistribution/DashBoardChannelDistributionChat"
import DashboardCampaignPhases from "app/creation/components/DashboardCampaignPhases"
import { colorClassToHex, isHexColor } from "components/Options"
...
const Dashboard = () => {
 ...
 const getFunnelColor = (stageName, campaign) => {
  const funnel = campaign?.custom_funnels?.find((f) => f.name === stageName);
  const color = funnel?.color || "bg-gray-500";
  return isHexColor(color) ? color : colorClassToHex[color] || "#c3c3c4";
 };
    ...
    return (
     ...
     >
      ...
      <div className="bg-[#F9FAFB] rounded-lg p-6 flex flex-col gap-4 min-h-[545px]">
       ...
       <DashBoardChannelDistributionChat
        campaign={campaign}
