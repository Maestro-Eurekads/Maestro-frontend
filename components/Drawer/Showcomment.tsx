...
// const Showcomment = ({ author = "John Doe", position, setShow }) => {
// 	const { addComment } = useComments(); // Get context functions
...
const Showcomment = ({ author = "John Doe", comment, setShow }) => {
	...
	const handleAddComment = () => {
		...
		addComment({ id: Date.now(), text, position: comment.position }); // Store at the correct position
		setText(""); // Reset input
	};
	...
	return (
		...
		<div className='cursor-move'>
			<div className="w-[395px] flex flex-col items-start p-[10px_20px] bg-white border border-black rounded-[8px]">
				<div className="flex justify-between items-center gap-3 w-full">
					<div className="flex items-center gap-2">
						<div className="flex flex-col justify-center items-center p-[10px] gap-[10px] w-[40px] min-h-[40px] bg-[#00A36C] rounded-full text-[20px] leading-[27px] text-center text-white">
							{author[0] || "John Doe"}
						</div>
						<CommentHeaderwithClose author={author} timestamp={new Date().toLocaleString()} />
					</div>
				...
				<div className="flex w-full justify-between">
					...
					<div>
						<button
							onClick={() => { setViewcommentsId(comment.id); setIsDrawerOpen(true) }}
							className="flex flex-row justify-center items-center px-[28px] py-[10px] gap-[8px] w-[135px] h-[40px] bg-[#3175FF] rounded-[8px] font-semibold text-[15px] leading-[20px] text-white cursor-pointer"
						>
							Comment
							<Image src={send} alt="send" />
