...
import ClientCommentHeaderwithMessage from "./ClientCommentHeaderwithMessage.tsx";
...
const ClientShowcomment = ({ comment, setActiveComment }) => {
  ...
  return (
    <div className="cursor-move">
      <div className="w-[395px] flex flex-col items-start p-[10px_20px] bg-white border border-black rounded-[8px] max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-center gap-3 w-full">
          <div className="flex items-center gap-2">
            ...
            <ClientCommentHeaderwithMessage
              author={cleanName(comment?.creator?.name)}
              comment={comment}
            />
          </div>
        ...
        <div className="flex w-full justify-between">
          {comment?.client_commentID === null ? (
            <div>
              <h3
                className={`font-semibold text-[15px] leading-[20px] text-[#00A36C] ${
