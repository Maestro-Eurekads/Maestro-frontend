...
const ViewClientModal = ({ isView, setIsView }) => {
  ...
  useEffect(() => {
    if (FC) {
      ...
      // Check if there are existing agency access users to set initial state
      const hasAgencyUsers = users.agencyAccess.length > 0;
      setIsAgencyAccessEnabled(hasAgencyUsers);
    }
  }, [FC, users.agencyAccess.length]);
  ...
  useEffect(() => {
    if (!isAgencyAccessEnabled) {
      setAgencyInput({ id: '', name: '', email: '', roles: [] });
      setShowAgencyInput(false);
      if (editingUser?.section === 'agencyAccess') {
        setEditingUser(null);
      }
    }
  }, [isAgencyAccessEnabled, editingUser]);
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-xs text-gray-500 mt-1 ml-7">
                      Check this box to manage agency users who can access this client
                    </p>
                  </div>
                  ...
                  <div className={`mt-4 transition-opacity duration-200 ${isAgencyAccessEnabled ? 'opacity-100' : 'opacity-50'}`}>
                    ...
                    <div className="flex items-start gap-3 w-full mt-2">
                      ...
                      <button
                        ...
                        disabled={!isAgencyAccessEnabled || loading || loadingUpdate}
                      >
                        {loadingUpdate ? (
                      ...
                      <button
                        className={`flex items-center justify-center px-6 py-3 w-[76px] h-[40px] rounded-lg font-semibold text-[14px] leading-[19px] mt-6 transition-colors ${isAgencyAccessEnabled
                    ...
                    {users.agencyAccess.length > 0 && isAgencyAccessEnabled && (
