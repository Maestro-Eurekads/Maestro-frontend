...
const ApprovalModals = () => {
  ...
  const internalApproverEmails =
    campaignFormData?.internal_approver?.map((a) => a?.email) || [];
  const isAssignedInternalApprover = internalApproverEmails.includes(
    loggedInUser.email
  );
  ...
  switch (effectiveStage) {
    case "draft":
      return isCreator || isNotApprover || isInternalApprover || isAdmin ? (
        <AskForApproval {...sharedProps} setChange={setChange} />
      ) : null;

    case "in_internal_review":
      return isAdmin || isAssignedInternalApprover ? (
        <InternallyApprovedModal {...sharedProps} setChange={setChange} />
      ) : null;

    case "internally_approved":
      return isAdmin || isAssignedInternalApprover ? (
        <ShareWithClientModal {...sharedProps} setChange={setChange} />
      ) : null;

    case "shared_with_client":
      return isAssignedInternalApprover || isAdmin ? (
        <SharedWithClientPromptModal {...sharedProps} />
      ) : (
        <ClientReviewModal {...sharedProps} />
      );
