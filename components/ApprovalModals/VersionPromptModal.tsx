...
const VersionPromptModal = () => {
  ...
  const sanitizeVersionData = (versionData) => {
    try {
      ...
      if (
        !cleanedData.campaign_version ||
        typeof cleanedData.campaign_version !== "string"
      ) {
  ...
  const getNextVersion = (currentVersion) => {
    // Sanitize input
    const sanitizedVersion = sanitizeVersionData({
      campaign_version: currentVersion,
    });
  ...
  const handleVersionChoice = async (choice?: string) => {
    ...
    try {
      ...
      if (
        ...
        (payload.data as any).ad_sets_granularity =
          cleanedFormData.ad_sets_granularity || cleanedFormData.granularity;
      }
      if (cleanedFormData?.campaign_timeline_start_date) {
        (payload.data as any).campaign_timeline_start_date =
          cleanedFormData.campaign_timeline_start_date;
      }
      if (cleanedFormData?.campaign_timeline_end_date) {
        (payload.data as any).campaign_timeline_end_date =
  ...
  const handleCreateNewVersion = async () => {
    ...
    try {
      ...
      const newVersion = campaignData?.id
      ...
      toast.success(
        campaignData?.id
          ? "New campaign version created successfully!"
      ...
      {showVersionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative bg-white rounded-lg w-[440px] max-w-full p-6 shadow-xl text-center">
            ...
            <p className="text-sm text-[#535862] mb-4">
              Would you like to create a new version or maintain the same
              version?
            </p>
