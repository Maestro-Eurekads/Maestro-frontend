...
const CampaignBudget = () => {
  ...
  const handleValidate = () => {
    if (budgetStyle === "top_down") {
      ...
      // Bottom-up: validate fees before proceeding to ConfigureAdSetsAndBudget
      if (campaignFormData?.campaign_budget?.budget_fees?.length > 0 || (!feeType && !feeAmount)) {
        setFeeStepValidated(true)
        setStep(3)
        return true
  ...
  useEffect(() => {
    if (campaignData?.campaign_budget) {
      ...
      if (campaignData?.campaign_budget?.budget_fees?.length > 0) {
        setFeeStepValidated(true)
        if (campaignData?.campaign_budget?.budget_type === "top_down") {
          setStep(2)
        } else {
          setStep(3)
        }
  ...
  return (
    <div>
      ...
      {/* Bottom-up flow: REVERSED LOGIC - Level selection first, then fees, then ConfigureAdSetsAndBudget last */}
      ...
      {/* Step 2: Fee Selection (moved before ConfigureAdSetsAndBudget in bottom-up) */}
      ...
      {/* Step 3: ConfigureAdSetsAndBudget (now comes last in bottom-up flow) */}
