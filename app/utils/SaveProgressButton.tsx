...
const SaveProgressButton = () => {
  ...
  const { isFinancialApprover, isAgencyApprover, isAdmin, loggedInUser } =
    useUserPrivileges();
      ...
      if (stageData) {
        ...
        const hasFormatSelected = channelTypes.some((channelType) => {
          ...
          return platforms.some((platform) => {
            ...
            const hasAdsetFormats = platform?.ad_sets?.some(
              (adset) =>
  ...
  useEffect(() => {
    if (
      active === 4 &&
      !hasProceededFromFormatStep.current &&
      !hasInitializedStep4.current
    ) {
      ...
      const hasExistingPreviews = campaignFormData.channel_mix?.some((mix) =>
        CHANNEL_TYPES.some(({ key }) =>
          mix[key]?.some((platform) => platform.format?.some(formatHasPreviews))
        )
      );
          ...
          await updateUsersWithCampaign(
            [
              ...(Array.isArray(loggedInUser?.id)
                ? loggedInUser?.id
                : [loggedInUser?.id]),
              ...(campaignFormData?.internal_approver || []).map((item: any) =>
                String(item.id)
              ),
              ...(campaignFormData?.client_approver || []).map((item: any) =>
                Number(item.id)
              ),
            ],
  ...
  const showConfirm =
    active === 10 &&
    (isInternalApprover
      ? isAdmin || internalApproverEmails.includes(loggedInUser.email)
      : false);
