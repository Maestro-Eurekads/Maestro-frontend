...
const TableView = () => {
  ...
  const mergeAdditionalKPIs = () => {
    ...
    setCampaignFormData((prev) => {
      ...
      updateCampaign({ table_headers: updatedData.table_headers });
      return updatedData;
    });
  };
  ...
  useEffect(() => {
    ...
    for (const [stageName, objectives] of Object.entries(stageObjectives)) {
      ...
      objectiveList?.forEach((objective) => {
        ...
        headers.forEach((h) =>
          headersSet.set(h.name, { ...h, obj: objective })
        );
  ...
  useEffect(() => {
    if (currentEditingStage && isOpen && !initializedRef.current) {
      ...
      console.log("cuurr", currentHeaders);
      // Find the objective for each existing metric
      currentHeaders.forEach((header) => {
        // First, prioritize looking through the existing tableHeaders for the current stage
        ...
        for (const objective of stageObjectives) {
          ...
          if (matchingMetric) {
            existingMetricsMap.set(header.name, {
              ...matchingMetric,
              obj: objective,
            });
        ...
        // If no match is found in the current stage's objectives, look through all objectives
      ...
      // console.log("here", Array.from(existingMetricsMap.values()));
  ...
  const existingHeaderNames = useMemo(() => {
    return currentEditingStage
      ? mergedTableHeadersByStage[currentEditingStage]?.map((h) => h.name)
      : [];
  }, [mergedTableHeadersByStage, currentEditingStage]);
          ...
          <div className="mt-4 max-h-[400px] overflow-y-auto">
            {(() => {
              ...
              const filteredObjectives = allObjectives.filter(
                (objective) =>
                  !objectivesForStage.includes(objective) &&
