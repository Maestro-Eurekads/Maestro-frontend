    ...
    try {
      ...
      if (existingUsers?.length > 0) {
        ...
        for (const emailEntry of inputs.agencyAccess) {
          try {
            await addClientUser({
              ...
              user_type: emailEntry?.roles === "Campaign Creator" ? "agency_creator" : emailEntry?.roles === "Agency Campaign Approver" ? "agency_approver" : "financial_approver",
              agencyId: agencyId,
              emailEntry
            }, jwt);
          } catch (error) {
        ...
        for (const emailEntry of inputs.clientAccess) {
          try {
            await addClientUser({
              ...
              password: "123456789",
              clients: res?.data?.data?.id,
              user_type: emailEntry?.roles === "Campaign Viewer" ? "client" : "client_approver",
              agencyId: agencyId,
              emailEntry
            }, jwt);
          } catch (error) {
        ...
        dispatch(getCreateClient({
          userId: res?.data?.data?.id, jwt, agencyId
        }));
        // Reset form state
        setInputs({
          name: "",
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-500">
                                    Role: {
                                      user.roles === "agency_creator" ? "Campaign Creator" :
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-xs text-gray-500">
                                      Role: {
                                        user.roles === "viewer" ? "Campaign Viewer" :
