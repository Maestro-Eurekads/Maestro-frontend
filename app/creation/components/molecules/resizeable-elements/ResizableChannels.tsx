...
const ResizableChannels = ({
  ...
  const { campaignFormData, setCampaignFormData, setCopy, cId } = useCampaigns()
  ...
  const sendUpdatedDataToAPI = async (updatedData) => {
    try {
      const response = await axios.put(
        ...
        {
          data: {
            ...removeKeysRecursively(updatedData, ["id", "documentId", "createdAt", "publishedAt", "updatedAt"]),
          },
        },
        {
          headers: {
        ...
        const newState = initialChannels.map((ch, index) => {
          ...
          return existingState
            ? {
                ...
                left: Math.min(parentLeft + Math.abs(startDateIndex), parentWidth),
                width:
                  daysBetween > 0
            ...
            {/* Controls */}
            {
              <>
                <div
                  ...
                  onMouseDown={disableDrag ? undefined : handleMouseDown(index, "left")}
                >
                ...
                >
                  ...
                  {!disableDrag && (
                    <button
                      ...
                      onClick={() => (disableDrag ? undefined : handleDeleteChannel(index))}
                    >
