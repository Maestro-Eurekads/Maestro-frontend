...
const ApproverContainer = ({ campaign, loading, isLoadingCampaign }) => {
  ...
  const internalApprovers = campaign?.internal_approver?.map(getApproverName) ||
    ...
    campaignData?.media_plan_details?.internal_approver?.map(
      getApproverName
    ) || ["-"];
  ...
  const items = [
    {
      ...
      name:
        cleanName(
          campaign?.client_selection?.value || campaignData?.client?.client_name
        ) || "-",
      nameList: [
        campaign?.client_selection?.value ||
    ...
    },
    {
      ...
      name:
        cleanName(
          campaign?.campaign_builder?.username ||
            campaign?.campaign_builder?.label ||
            campaignData?.campaign_builder?.username ||
            campaignData?.campaign_builder?.label
        ) || "-",
      nameList: [
        campaign?.campaign_builder?.username ||
          campaign?.campaign_builder?.label ||
