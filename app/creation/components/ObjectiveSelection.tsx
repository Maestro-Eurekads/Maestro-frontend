...
// Replicate the same platforms value for all stages.
const platformsData = {
  "Social media": [
    { name: "Facebook", icon: facebook },
    { name: "Buy type" },
...
const ObjectiveSelection = () => {
  ...
  // Toggle the dropdown for a given interactive platform field only if stage is not completed
  const toggleDropdown = (platformKey: string, stageStatus: string) => {
  ...
  // Return dropdown options based on interactive platform name
  const getDropdownOptions = (platform: { name: string; icon?: unknown }) => {
    if (platform.name === "Facebook" || platform.name === "Instagram" || platform.name === "Youtube") {
      return ["Awareness", "CPM"];
    }
    if (platform.name === "TheTradeDesk" || platform.name === "Quantcast") {
      return ["Video views", "CPV"];
    }
  ...
  return (
    <div className="mt-12 flex items-start flex-col gap-12 w-full max-w-[950px]">
      <Toaster position="top-right" reverseOrder={false} />
      {funnelStages.map((stage, index) => {
        // Compute required field keys for interactive fields
        const requiredFieldKeys: string[] = [];
        Object.entries(stage.platforms).forEach(([category, platforms]) => {
          platforms.forEach((platform, pIndex) => {
            if (platform.icon || platform.name === "Buy type" || platform.name === "Buy objective") {
              requiredFieldKeys.push(`${stage.name}-${category}-${pIndex}`);
            }
          });
        });
        const allRequiredSelected = requiredFieldKeys.every(
          ...
          <div key={index} className="w-full">
            ...
            {openItems[stage.name] && (
              <div className="flex items-start flex-col gap-8 p-6 bg-white border border-gray-300 rounded-b-lg">
                {Object.entries(stage.platforms).map(([category, platforms]) => (
                  <div key={category} className="flex flex-col items-start gap-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                      {platforms.map((platform, pIndex) => {
                        const platformKey = `${stage.name}-${category}-${pIndex}`;
                        // Render interactive dropdown for fields with an icon or for Buy type/objective
                        if (platform.icon || platform.name === "Buy type" || platform.name === "Buy objective") {
                          return (
                            <div key={pIndex} className="relative w-full">
                              <div
                                className="flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg cursor-pointer"
                                onClick={() => toggleDropdown(platformKey, statuses[index])}
                              >
                                <div className="flex items-center gap-3">
                                  {platform.icon && (
                                    <Image
                                      className="flex flex-shrink-0 size-4"
                                      src={platform.icon}
                                      alt={platform.name}
                                    />
                                  )}
                              ...
                              {/* Dropdown Menu (only if stage is not completed) */}
                              {dropdownOpen[platformKey] && statuses[index] !== "Completed" && (
                                <div className="absolute left-0 mt-2 w-full bg-white border border-gray-300 rounded-md shadow-lg transition-transform transform hover:scale-105 z-10">
                                  <ul className="py-2">
                                    {getDropdownOptions(platform).map((option, i) => (
                                      <li
                ...
                {/* Validate Button (shown if interactive fields exist) */}
                {requiredFieldKeys.length > 0 && statuses[index] !== "Completed" && (
                  <div className="flex justify-end mt-6 w-full">
                    <Button
                      text="Validate"
                      variant="primary"
