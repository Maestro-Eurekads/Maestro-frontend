...
import { addDays, differenceInCalendarDays, eachDayOfInterval, isEqual, parseISO, subDays } from "date-fns"
...
const ResizableChannels = ({
  ...
  const { campaignFormData, setCampaignFormData, setCopy, cId, campaignData } = useCampaigns()
  ...
  const handleDeleteChannel = async (indexToDelete) => {
    ...
    const updatedCampaignFormData = JSON.parse(JSON.stringify(campaignFormData)) // Deep copy
      ...
      setChannelState((prev) =>
        prev.map((state, i) => {
          ...
          if (direction === "left") {
            ...
            const snappedRightEdge = snapToTimeline(rightEdge - parentLeft, parentWidth) + parentLeft

            // Calculate new width based on snapped right edge
            newWidth = Math.max(150, Math.min(snappedRightEdge - state.left, parentWidth - (state.left - parentLeft)))
          }
    ...
    >
                ...
                {disableDrag && (
                  ...
                  >
                    {isNaN(Number(budget)) || Number(budget) <= 0
                      ? ""
                      : `${Number(budget)?.toFixed(0)} ${getCurrencySymbol(campaignFormData?.campaign_budget?.amount)}`}
                  </div>
                )}
              </div>
            </div>
            {/* Controls */}
            {
              <>
                <div
                  ...
                  onMouseDown={disableDrag || openItems ? undefined : handleMouseDown(index, "left")}
                >
                ...
                <div
                  ...
                  onMouseDown={disableDrag || openItems ? undefined : handleMouseDown(index, "right")}
                >
                  <MdDragHandle className="rotate-90" />
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <Image
                          src={
                            openItems &&
                            openItems === `${channel?.name || "/placeholder.svg"}${index}`
                              ? arrowUp
                              : arrowDown
                          }
                          alt=""
                        ...
                        {channel?.ad_sets?.map((set, index) => (
                          <React.Fragment key={index}>
                            <tr className="border-none">
                              ...
                              <td className="px-4 py-2 whitespace-nowrap border-none">{set?.size}</td>
                            </tr>
                            {set?.extra_audiences?.map((extra, extraIndex) => (
                              <tr key={`${index}-${extraIndex}`} className="border-none">
                                ...
                                <td className="px-4 py-2 whitespace-nowrap border-none">{extra?.size}</td>
                              </tr>
                            ))}
                          </React.Fragment>
                        ))}
      ...
      <Modal isOpen={openCreatives} onClose={() => setOpenCreatives(false)}>
        <div className="bg-white w-[900px] p-6 rounded-lg h-[600px] overflow-y-auto">
          ...
          {disableDrag ? "" : <FormatSelection stageName={parentId} platformName={selectedChannel} />}
          {/*
           */}
        </div>
      </Modal>
      <Modal isOpen={openAdset} onClose={() => setOpenAdset(false)}>
        <div className="bg-white w-[900px] p-2 rounded-lg">
          ...
          <div className="w-fit ml-auto">
            <button className="bg-blue-500 text-white rounded-md p-2" onClick={() => setOpenAdset(false)}>
              Confirm Changes
            </button>
          </div>
        </div>
