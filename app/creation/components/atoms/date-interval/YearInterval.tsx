...
const YearInterval: React.FC<YearIntervalProps> = ({
  ...
  useEffect(() => {
    if (campaignFormData) {
      ...
      const differenceInDaysCount = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24))
      ...
      const years = Array.from(new Set(dateList.map((date) => format(date, "yyyy"))))
      setYearNames(years)
  ...
  return (
    <div className="w-full border-y">
      ...
      >
        {Object.entries(daysInYear || {}).map(([yearName, dayCount], i) => (
          <div key={i} className="flex flex-col items-center relative py-4 border-r-2 border-blue-300 last:border-r-0">
