...
const OldMonthInterval: React.FC<MonthIntervalProps> = ({
  monthsCount,
  view,
  getDaysInEachMonth,
  funnelData,
    ...
    <div className="w-full border-y py-1">
      ...
      >
        {headerMonths.map((monthDate, i) => {
          ...
          const daysInMonthInRange =
            range?.filter((date) => date >= monthStart && date <= monthEnd)
              .length || 0;

          // Calculate the grid column span for this month
          const gridColumnSpan =
            daysInMonthInRange > 0 ? daysInMonthInRange : 1;
          ...
          for (let j = 0; j < i; j++) {
            ...
            const prevDaysInRange =
              range?.filter(
                (date) => date >= prevMonthStart && date <= prevMonthEnd
              ).length || 0;
...
export default OldMonthInterval;
