...
export const validationRules = {
  ...
  step3: (data) =>
    !!data?.channel_mix && Object.keys(data.channel_mix).length > 0,
...
interface VerificationContextType {
  ...
  setverifybeforeMove: React.Dispatch<
    React.SetStateAction<VerifyBeforeMoveState>
  >;
  hasChanges: boolean;
...
export const VerificationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [verifybeforeMove, setverifybeforeMove] =
    useState<VerifyBeforeMoveState>(loadStoredState);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      localStorage.setItem(
        `verifybeforeMove_${campaignId}`,
        JSON.stringify(verifybeforeMove)
      );
    }
  }, [verifybeforeMove]);
