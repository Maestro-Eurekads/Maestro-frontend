...
const AdSet = memo(function AdSet({
  ...
  const handleAddNewAudience = () => {
    if (canAddNewAudience) {
      ...
      toast("Fill in previous audience details first");
    }
  };
...
const AudienceDropdownWithCallback = memo(
  function AudienceDropdownWithCallback({
    ...
    const { customAudienceTypes, addCustomAudienceType } = useContext(
      CustomAudienceTypesContext
    );
      ...
      Object.keys(storedState).forEach((stageName) => {
        ...
        Object.keys(storedState[stageName]).forEach((platformName) => {
          channelLevelAudienceState[stageName][platformName] =
            storedState[stageName][platformName];
        });
      });
                ...
                {adsets.map((adset, index) => (
                  ...
                  >
                    <AdSet
                      ...
                      onAddNewAdSet={
                        granularity === "adset" ? addNewAddset : undefined
                      }
                    />
                  </div>
                ))}
        ...
        <div className="mt-2 mb-4">
          <div className="bg-[#F5F7FA] border border-[#E5E7EB] rounded-lg px-4 py-3">
            <div className="font-semibold text-[#061237] mb-2 text-sm">
              Audience Recap (
              {granularity === "channel" ? "Channel Level" : "Ad Set Level"})
            </div>
    ...
    >
      <div
        className={`w-full p-4 ${
          granularity === "channel" ? "space-y-4" : "space-y-4"
        }`}
        style={
          granularity === "channel"
            ? {
