...
const defaultFunnels = [{ name: "Awareness" }, { name: "Consideration" }, { name: "Conversion" }]
...
export default function PlatformSpending(insideText) {
  ...
  const extractPhasesData = (channel_data: any[] | undefined): Phase[] => {
    ...
    channel_data.forEach((stage) => {
      ...
      ;[
        ...
        stage[channelType].forEach((platform: any) => {
          ...
          if (!platformMap[platformName]) {
            const style = platformStyles.find((style) => style.name === platformName) ||
      ...
      // Get the stage budget - this is the key fix
      const stageBudgetValue = stage?.stage_budget?.fixed_value ? Number(stage.stage_budget.fixed_value) : 0

      // Calculate percentages based on stage budget, not sum of platform budgets
      const platformList = Object.values(platformMap)
        .filter((p) => p.value > 0)
        .map((p) => ({
          ...p,
          // Use stage budget as denominator for percentage calculation
          value: stageBudgetValue > 0 ? Math.round((p.value / stageBudgetValue) * 100) : 0,
        }))
