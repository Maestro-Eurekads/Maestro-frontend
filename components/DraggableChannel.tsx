...
const DraggableChannel: React.FC<DraggableChannelProps> = ({
  ...
  function calculateDailyWidth(containerWidth: number, endMonth: number): number {
  ...
  const snapToTimeline = (currentPosition: number, containerWidth: number) => {
    ...
    while (currentSnap <= containerWidth) {
      ...
      currentSnap += step
      step = Math.max(range === "Month" ? dailyWidth : 50, step - adjustmentPerStep)
    }
  ...
  const handleMouseMoveResize = (e: MouseEvent) => {
    ...
    if (updatedChannelMix) {
      ...
      setCampaignFormData((prev) => ({
        ...prev,
        channel_mix: prev.channel_mix.map((ch) => (ch.funnel_stage === description ? updatedChannelMix : ch)),
      }))
    }
  ...
  return (
    ...
    >
      ...
      {/* Draggable Content */}
      <div
        className={`${color} h-full flex justify-between items-center text-white px-4 py-[10px] gap-2 border shadow-md min-w-[50px] ${
