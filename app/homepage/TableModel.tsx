...
const TableModel = ({ isOpen, setIsOpen }) => {
 ...
 const handleAddAgencyAccess = () => {
  ...
  if (
   editingIndex === null &&
   inputs.agencyAccess.some((item) => item.email.toLowerCase() === trimmedEmail)
  ) {
 ...
 const handleAddClientAccess = () => {
  ...
  if (
   editingIndex === null &&
   inputs.clientAccess.some((item) => item.email.toLowerCase() === trimmedEmail)
  ) {
 ...
 const handleSubmit = async () => {
  ...
  try {
   // Convert all emails to lowercase before checking and submitting
   const allEmails = [...inputs.agencyAccess, ...inputs.clientAccess].map((e) => e.email.trim().toLowerCase());
   const agencyAccessLower = inputs.agencyAccess.map((entry) => ({ ...entry, email: entry.email.trim().toLowerCase() }));
   const clientAccessLower = inputs.clientAccess.map((entry) => ({ ...entry, email: entry.email.trim().toLowerCase() }));
   ...
   if (existingUsers?.length > 0) {
    ...
    for (const emailEntry of agencyAccessLower) {
     try {
      await addClientUser(
       {
    ...
    for (const emailEntry of clientAccessLower) {
     try {
      await addClientUser(
       {
        username: `${emailEntry.name}-${uuidv4().slice(0, 4)}`,
