...
const ComfirmModel = ({ isOpen, setIsOpen }) => {
  ...
  const [step, setStep] = useState<
    "creator" | "internal_approver" | "client" | null
  >(null);
  ...
  // Add validateAndFormatDates helper
  ...
  const handleCreateNewVersion = async () => {
    ...
    try {
      ...
      const currentVersionNumber = Number(
        (campaignData?.campaign_version || "V1").replace("V", "")
      );
  ...
  const isAssignedInternalApprover =
    isInternalApprover &&
    campaignData?.media_plan_details?.internal_approver?.includes(
      String(loggedInUser?.id)
    );
  const isAssignedClientApprover =
    isClient &&
    campaignData?.media_plan_details?.client_approver?.includes(
      String(loggedInUser?.id)
    );
  const isNotApprover =
    !campaignData?.media_plan_details?.internal_approver?.includes(
  ...
  useEffect(() => {
    ...
    if (!stage || stage === "draft") {
      if (isCreator || isNotApprover || isInternalApprover || isAdmin) {
        ...
        toast.error(
          "Only the campaign creator who is not an approver can request approval."
        );
        setIsOpen(false);
      }
      return;
    ...
    if (isInternalApprover || isAdmin) {
      ...
      setTitle("Review Media Plan");
      setStatusMessage(
        "Do you want to approve the media plan or request changes?"
      );
    } else {
  ...
  const handleSaveWithVersion = async () => {
    ...
    toast.info(
      "Please choose to maintain the same version or create a new one."
    );
    setShowVersionPrompt(true);
  };
  ...
  const handleRequestChanges = async () => {
    ...
    const changeStage =
      step === "client" ? "client_changes_needed" : "changes_needed";
    const label =
      step === "client" ? "Client Changes Needed" : "Changes Needed";
  ...
  return (
    <>
          ...
          <div className="relative bg-white rounded-lg w-[440px] max-w-full p-6 shadow-xl text-center">
            ...
            <div className="text-sm text-[#535862] mb-4 text-left">
              <p>
                ...
                {campaignData?.media_plan_details?.plan_name || "N/A"}
              </p>
              <p>
                <strong>Current Version:</strong> Media Plan V
                {campaignData?.campaign_version || "1"}
              </p>
              <p>
                <strong>Status:</strong>{" "}
                {campaignData?.isStatus?.label || "Draft"}
              </p>
              <p>
            ...
            {campaignData?.campaign_version && (
              <p className="text-xs text-[#717A8C] mt-1">
                ...
                <span className="font-semibold">
                  Media Plan V{campaignData.campaign_version}
                </span>
              </p>
            )}
      ...
      {showVersionPrompt ? null : stage === "shared_with_client" ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative bg-white rounded-lg w-[440px] max-w-full p-6 shadow-xl text-center">
            ...
            <p className="text-sm text-[#535862] mb-4">
              The media plan has been shared with the client and is under
              review.
            </p>
      ...
      ) : (stage === "internally_approved" || step === "internal_approver") &&
        showSharePrompt ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative bg-white rounded-lg w-[440px] max-w-full p-6 shadow-xl text-center">
            <button
            ...
            <p className="text-sm text-[#535862] mb-4">
              Do you want to share with the client now?
            </p>
            <div className="flex flex-col gap-4">
              <button
                className="btn_model_active w-full"
