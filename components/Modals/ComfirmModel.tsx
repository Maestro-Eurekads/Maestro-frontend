...
// 								{step === 'internal_approver' && (isAdmin || isFinancialApprover ? 'Approve Internally' : 'Share with Client')}
// 								{step === 'client' && 'Approve'}
// 							</span>
// 						}
// 					</button>
...
const ComfirmModel = ({ isOpen, setIsOpen }) => {
	...
	useEffect(() => {
		...
		const lastStatus = campaignData?.isStatus;
	...
	const updateStatus = async (stage, label) => {
		...
		try {
			...
			await axios.put(`${process.env.NEXT_PUBLIC_STRAPI_URL}/campaigns/${cId}`, {
				data: {
					isStatus: newStatus, // <== now storing a single object
				}
			}, {
				headers: {
					Authorization: `Bearer ${jwt}`,
