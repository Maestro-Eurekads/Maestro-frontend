...
// components/FiltersDropdowns.jsx
"use client";
import { useState, useRef, useEffect, useMemo } from "react";
import down from "../../../public/down.svg";
import Image from "next/image";
...
const FiltersDropdowns = ({ hideTitle, router }: Props) => {
  ...
  const allFiltersEmpty: any = useMemo(
    () => Object.values(selectedFilters).every((val) => !val || (typeof val === "object" && !val?.name)),
    [selectedFilters]
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = allFiltersEmpty
          ? await fetchFilteredCampaigns(clientID, {})
    ...
    fetchData();
  }, [selectedFilters, allClients, userType]);
