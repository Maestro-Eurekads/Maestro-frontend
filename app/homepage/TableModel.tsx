...
//       setAgencyInput((prev) => ({
//         ...prev,
//         roles: [role], // Set only the selected role
//       }));
//     } else {
//       setClientInput((prev) => ({
//         ...prev,
...
//       const existingUsers = await checkExisitingEmails(allEmails);
//       if (existingUsers?.length > 0) {
//         toast.error(
//           `User(s) with the following email(s) already exist: ${existingUsers
//             .map((user) => user.email)
...
const TableModel = ({ isOpen, setIsOpen }) => {
  ...
  console.log('inputs-inputs-inputs', inputs)
  ...
  const handleAddAgencyAccess = () => {
    ...
    if (!roles) {
      toast.error("A role must be selected for Agency Access");
      return;
    }
    ...
    setInputs((prev) => ({
      ...prev,
      agencyAccess: [...prev.agencyAccess, { name: `${title} ${trimmedName}`, email: trimmedEmail, roles }],
    }));
  ...
  const handleAddClientAccess = () => {
    ...
    setInputs((prev) => ({
      ...prev,
      clientAccess: [...prev.clientAccess, { name: `${title} ${trimmedName}`, email: trimmedEmail, roles }],
    }));
  ...
  const handleRoleChange = (section, role) => {
    if (section === "agency") {
      setAgencyInput((prev) => ({
        ...prev,
        roles: role, // Set the selected role as a string
      }));
    } else {
      setClientInput((prev) => ({
        ...prev,
  ...
  const accessGroups = [
    {
      list: inputs.agencyAccess,
      user_type: agencyInput.roles,
      title: agencyInput.title, // Use agency title
    },
    {
      list: inputs.clientAccess,
      user_type: clientInput.roles,
      title: clientInput.title, // Use client title
    },
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Call to create client first
      const res = await addNewClient({
        client_name: inputs.name,
        client_emails: inputs.clientAccess.map((e) => ({
          email: e.email,
          full_name: e.name,
        })),
        responsible: [], // Will populate after user creation
        approver: [], // Will populate after user creation
        level_1: inputs.sports,
        level_2: inputs.businessUnits,
        level_3: inputs.categories,
        users: profile?.id,
      ...
      localStorage.setItem(userType.toString(), clientId);
      getProfile();
      ...
      for (const group of accessGroups) {
        for (const entry of group.list) {
          try {
            const userRes = await addClientUser({
              username: `${entry.name}-${uuidv4().slice(0, 4)}`,
              email: entry.email,
              password: "12345678",
              title: group.title || "",
            ...
            console.error(`Failed to create user for ${entry.email}`, error);
            toast.error(`Failed to create user for ${entry.email}`);
          }
        }
      }

      // Now that all users are created, extract IDs
      const responsibleIds = createdUsers
        .filter((u) => u.user_type === "agency")
        .map((u) => u.id);

      const approverIds = createdUsers
        .filter((u) => u.user_type === "sub_client")
        .map((u) => u.id);

      // Update client with responsible and approver IDs
      await updateClient(clientId, {
        responsible: responsibleIds,
        approver: approverIds,
      });

      dispatch(getCreateClient(!isAdmin ? clientId : null));
                ...
                <div className="flex items-start gap-3 w-full mt-2">
                  <div className="flex flex-col items-start gap-1 w-full">
                    <div className="shrink-0 w-[20%]">
                      <select
                        value={agencyInput.title}
                        onChange={(e) =>
                          setAgencyInput((prev) => ({ ...prev, title: e.target.value }))
                        }
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-500">{user.email}</p>
                                  <p className="text-xs text-gray-500">
                                    Role: {user.roles === "agency_creator" ? "Campaign Creator" :
                                      user.roles === "agency_approver" ? "Agency Campaign Approver" :
                                        "Financial Approver"}
                                  </p>
                                </div>
                              </div>
                  ...
                  <div className="flex flex-col items-start gap-1 w-full">
                    <div className="shrink-0 w-[20%]">
                      <select
                        ...
                        onChange={(e) =>
                          setClientInput((prev) => ({ ...prev, title: e.target.value }))
                        }
                        className="w-full p-2 border border-gray-300 rounded-lg"
                      >
                        {titleOptions.map((title) => (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-500">{user.email}</p>
                                  <p className="text-xs text-gray-500">
                                    Role: {user.roles === "viewer" ? "Campaign Viewer" :
