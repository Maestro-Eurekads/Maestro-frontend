...
const MultiSelectDropdown = ({
	...
	islabelone: string;
	islabeltwo: string;
	onSelect: (values: { id: string; username: string }[]) => void;
}) => {
	const [selectedOptions, setSelectedOptions] = useState<{ id: string; username: string }[]>([]);
	const [isOpen, setIsOpen] = useState(false);
	const dropdownRef = useRef<HTMLDivElement>(null);

	const toggleDropdown = () => setIsOpen((prev) => !prev);

	const handleSelect = (option: { id: string; username: string }) => {
	...
	const handleRemove = (option: { id: string }) => {
		const updated = selectedOptions.filter((item) => item.id !== option.id);
		setSelectedOptions(updated);
		onSelect(updated);
	};
...
const ResponsibleApproverDropdowns = ({
	...
	right: boolean;
	options: { id: string; username: string }[];
	option: { id: string; username: string }[];
	setInputs: React.Dispatch<
		React.SetStateAction<{
			responsiblePerson?: string[];
			approver?: string[];
	...
	const handleSelect = (field: "responsiblePerson" | "approver", values: { id: string }[]) => {
		setInputs((prevState) => ({
			...prevState,
			[field]: values.map((val) => val.id), // Just pass IDs to state
		}));
	};
