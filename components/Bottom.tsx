...
const Bottom = ({ setIsOpen }: BottomProps) => {
  ...
  const handleContinue = async () => {
    ...
    // ✅ Step Three Validation - Ensure at least one channel is selected and validated for each funnel stage
    if (active === 3) {
      const hasUnvalidatedStages = campaignFormData?.funnel_stages?.some((stage) => {
        const isStageValidated = campaignFormData.validatedStages?.[stage];
        return !isStageValidated;
      });

      if (hasUnvalidatedStages) {
    ...
    <footer id="footer" className="w-full">
      ...
      {triggerChannelMixError && (
        <AlertMain
          alert={{
            variant: "error",
            message: "Please select and validate at least one channel for each funnel stage!",
            position: "bottom-right",
          }}
        />
      )}
