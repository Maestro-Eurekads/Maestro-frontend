  ...
  return (
    <div className="w-full space-y-4 p-4">
      {platforms[stageName]?.map((outlet) => (
        <AdsetSettings key={outlet.id} outlet={outlet} stageName={stageName} onInteraction={handleInteraction} />
      ))}
      {isEditing && (
        <div className="flex justify-end gap-2 w-full">
          <button
            ...
            className={`w-[142px] h-[52px] text-white px-6 py-3 rounded-md text-sm font-bold ${
              !hasInteraction ? "bg-blue-300 cursor-not-allowed" : "bg-[#3175FF] hover:bg-blue-600"
            }`}
          >
            <span>Validate</span>
          </button>
