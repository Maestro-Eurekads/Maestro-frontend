...
const ObjectiveSelection = () => {
  ...
  const [selectedOptions, setSelectedOptions] = useState(() => {
    // Initialize with empty object, will be loaded in useEffect with campaign-scoped key
    return {};
  });
  // Remove isEditable state
  ...
  // Remove validatedPlatforms state
  ...
  // Helper function to get campaign-scoped localStorage key
  const getCampaignScopedKey = (baseKey: string) => {
    const currentCampaignId = campaignFormData?.cId || "default";
    return `${baseKey}_${currentCampaignId}`;
  };
  ...
  useEffect(() => {
    ...
    // Use campaign-scoped localStorage keys
    const storedStatuses = localStorage.getItem(
      `funnelStageStatuses_${currentCampaignId}`
    );
    const storedSeenStages = localStorage.getItem(
      `seenFunnelStages_${currentCampaignId}`
    );
    const storedOptions = localStorage.getItem(
      `selectedOptions_${currentCampaignId}`
    );
    // Remove validatedPlatforms from localStorage logic
    ...
    if (currentPlanId !== currentPlanIdRef.current) {
      ...
      localStorage.removeItem(`funnelStageStatuses_${currentCampaignId}`);
      localStorage.removeItem(`seenFunnelStages_${currentCampaignId}`);
      localStorage.removeItem(`selectedOptions_${currentCampaignId}`);
      // localStorage.removeItem("validatedPlatforms")
      seenStagesRef.current = new Set();
      setStatuses({});
      setSelectedOptions({});
      setPreviousSelectedOptions({});
      // setValidatedPlatforms({ Awareness: new Set(), ... })
      // setIsEditable({})
      ...
      // Remove validatedPlatforms loading
      // if (storedPlatforms) { ... }
    }
    ...
    if (campaignFormData?.funnel_stages) {
      ...
      if (shouldUpdateStorage) {
        localStorage.setItem(
          `seenFunnelStages_${currentCampaignId}`,
          JSON.stringify(Array.from(seenStagesRef.current))
        );
      }
    }
  }, [campaignFormData?.planId, campaignFormData?.funnel_stages]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      getCampaignScopedKey("selectedOptions"),
  ...
  useEffect(() => {
    if (campaignFormData?.funnel_stages) {
      ...
      localStorage.setItem(
        getCampaignScopedKey("funnelStageStatuses"),
        JSON.stringify(updatedStatuses)
      );
    }
  }, [selectedOptions, campaignFormData?.funnel_stages]);
  ...
  // Remove validatedPlatforms localStorage effect

  // Remove force context update after validation

  const toggleItem = (stage) => {
    setOpenItems((prev) => ({ ...prev, [stage]: !prev[stage] }));
  };

  // Remove auto-validation logic from toggleDropdown
  ...
  // Remove handleEdit and all edit logic
  ...
  const hasCompleteSelection = (stageName) => {
    ...
    return Array.from(selectedNetworks[stageName]).some((platformName) => {
      ...
      return categories.some((category) =>
        hasCompletePlatformSelection(platformName, category, stageName)
      );
    });
  };

  // Remove renderCompletedPlatform and hasValidatedPlatformsForCategory (and their usage)
  ...
  const handleSaveCustomValue = async (field) => {
    ...
    try {
      ...
      if (field === "obj") {
        ...
        toast.success(
          `Custom buy objective "${customValue}" added successfully!`
        );
      } else {
  ...
  const getStageRecap = (stageName) => {
    ...
    categories.forEach((category) => {
      const platforms = Array.isArray(stageData[category])
        ? stageData[category]
        : [];
      platforms.forEach((platform) => {
        const buyType =
        ...
        if (buyType || objectiveType) {
          recapArr.push(
            ...
            }</strong>: <strong>Objective</strong>: ${
              objectiveType || "No objective"
            }, <strong>Buy Type</strong>: ${buyType || "No buy type"}`
          );
        }
      });
    });
            ...
            {!openItems[stage.name] && (
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: `<span class="font-semibold">Recap: </span>${getStageRecap(
                    stageName
                  )}`,
                }}
              />
