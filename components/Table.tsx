...
const Table = () => {
  ...
  const currentItems = clientCampaignData?.slice(indexOfFirstItem, indexOfLastItem) || []
  const totalPages = Math.ceil((clientCampaignData?.length || 0) / itemsPerPage)
  ...
  const handleDuplicateAction = async (e: any) => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        setDuplicateName("")
        setSelected({})
        updateOrignalCmapignCount(selected?.documentId, selected?.copyCount + 1)
      })
      .catch((err) => {
        console.log(err)
      })
              ...
              currentItems.map((data) => (
                <tr
                  ...
                  onClick={() => {
                    ...
                    const activeStepFromPercentage = Math.ceil((data?.progress_percent * 9) / 100)
                ...
                >
                  <td className="whitespace-nowrap py-[12px] px-[16px]">
                    {data?.media_plan_details?.plan_name} - {data?.progress_percent < 100 ? "Running" : "Completed"}
                  </td>
                  ...
                  <td className="py-[12px] px-[16px]">
                    ...
                    {!data?.budget_details?.currency?.includes("%") && data?.budget_details?.currency?.includes("EUR")
                      ? "€"
                      : ""}
                  </td>
                  <td className="py-[12px] px-[16px]">
                  ...
                  <td className="py-[12px] px-[16px]">
                    <div className="flex gap-4" onClick={(e) => e.stopPropagation()}>
                      ...
                      />
                      <Image src={share || "/placeholder.svg"} alt="menu" onClick={(e) => e.stopPropagation()} />
                      <Image
      ...
      {!loading && clientCampaignData?.length > 0 && (
        <div className="flex items-center justify-between mt-4 px-4">
          <div className="text-sm text-gray-600">
            Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, clientCampaignData.length)} of{" "}
          ...
          <div className="flex items-center space-x-2">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
              <button
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-md ${
                  currentPage === number ? "bg-blue-500 text-white" : "text-gray-700 hover:bg-blue-50"
                }`}
              >
                {number}
              </button>
            ...
            <button
              ...
              className={`p-2 rounded-md ${currentPage === totalPages ? "text-gray-400 cursor-not-allowed" : "text-blue-500 hover:bg-blue-50"}`}
            >
        ...
        <Modal isOpen={openModal === "copy"} onClose={() => setOpenModal("")}>
          <div className="bg-white shadow-md rounded-md w-[500px] p-4">
            ...
            <form onSubmit={handleDuplicateAction}>
              <label htmlFor="" className="font-medium flex items-center gap-[5px]">
                New Media Plan Name <span className="text-[14px] text-slate-400">(optional)</span>
              </label>
              <input
                type="text"
