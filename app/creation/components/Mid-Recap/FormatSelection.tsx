...
export const FormatSelectionsSection: React.FC<FormatSelectionsSectionProps> = ({ platforms }) => {
  return (
                      ...
                      <div className="text-sm text-gray-600">
                        {platform.formats.map((format, index) => (
                          <div key={index} className="mb-1">
                            <div className="font-semibold">{format.format_type}</div>
                            <div className="font-semibold">Number of visuals - {format.num_of_visuals}</div>
                            {format?.previews?.length > 0 &&
                              format?.previews?.map((preview, idx) => (
                                <div key={idx} className="mb-1 flex gap-3">
                                  <Link href={preview?.url} target="_blank" className="font-semibold underline">
                                    Preview {idx + 1}
                                  </Link>
                                </div>
                              ))}
